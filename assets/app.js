...
setLanguage(localStorage.getItem("libreart-language") || "en");
...
function getIssuesUrl() {
  ...
  if (hostname.endsWith(".github.io")) {
    ...
    const repository = window.location.pathname.split("/").filter(Boolean)[0] || "libreart";
    return `https://github.com/${owner}/${repository}/issues/new`;
  }
  return "https://github.com/SEU_USUARIO/libreart/issues/new";
}
...
form.addEventListener("submit", (event) => {
  ...
  if (!imageUrl.startsWith("https://pasteboard.co/")) {
    const language = localStorage.getItem("libreart-language") || "en";
  ...
  const issueBody = [
    ...
    "",
    "I confirm that this submission follows the archive's authorship and transparency guidelines."
  ].join("\\n");
  const issueUrl = `${getIssuesUrl()}?title=${encodeURIComponent(`[Submission] ${title}`)}&body=${encodeURIComponent(issueBody)}`;
